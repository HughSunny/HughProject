...
class ItemMainLayout extends FrameLayout {
    ...
    /* 子view */
    private ItemBackgroudLayout mItemLeftBackGroundLayout;
    private ItemBackgroudLayout mItemRightBackGroundLayout;
    ...
    protected ItemMainLayout(Context context, View customView) {
        ...
        mItemRightBackGroundLayout = new ItemBackgroudLayout(context);
        addView(mItemRightBackGroundLayout, new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
        mItemLeftBackGroundLayout = new ItemBackgroudLayout(context);
     ...
     * @return
     */
    protected ItemBackgroudLayout getItemLeftBackGroundLayout() {
     ...
     * @return
     */
    protected ItemBackgroudLayout getItemRightBackGroundLayout() {
    ...
    protected void setSelector(Drawable drawable) {
        UIUtil.setBackgroundDrawable(mItemLeftBackGroundLayout, drawable);
        UIUtil.setBackgroundDrawable(mItemRightBackGroundLayout, drawable);
    }
                ...
                switch (mIntention) {
                    ...
                    case INTENTION_LEFT_ALREADY_OPEN:
                        ...
                        if (Math.abs(mItemCustomView.getLeft()) > mBtnLeftTotalWidth / 2) {
                            ...
                            if (mOnItemSlideListenerProxy != null && mScrollState != SCROLL_STATE_OPEN) {
                                mOnItemSlideListenerProxy.onSlideOpen(this, ListMenuItem.DIRECTION_LEFT);
                            }
                            ...
                            if (mOnItemSlideListenerProxy != null && mScrollState != SCROLL_STATE_CLOSE) {
                                mOnItemSlideListenerProxy.onSlideClose(this, ListMenuItem.DIRECTION_LEFT);
                            }
                    ...
                    case INTENTION_RIGHT_ALREADY_OPEN:
                        if (Math.abs(mItemCustomView.getLeft()) > mBtnRightTotalWidth / 2) {
                            ...
                            mScroller.startScroll(mItemCustomView.getLeft(), 0, -delta, 0, SCROLL_TIME);
                            if (mOnItemSlideListenerProxy != null && mScrollState != SCROLL_STATE_OPEN) {
                                mOnItemSlideListenerProxy.onSlideOpen(this, ListMenuItem.DIRECTION_RIGHT);
                            }
                            mScrollState = SCROLL_STATE_OPEN;
                        } else {
                            mIntention = INTENTION_RIGHT_CLOSE;
                            ...
                            //滑回去,归位
                            if (mOnItemSlideListenerProxy != null && mScrollState != SCROLL_STATE_CLOSE) {
                                mOnItemSlideListenerProxy.onSlideClose(this, ListMenuItem.DIRECTION_RIGHT);
                            }
                            mScrollState = SCROLL_STATE_CLOSE;
                        }
                        break;
    ...
    protected void scrollBack() {
        ...
        if (mOnItemSlideListenerProxy != null && mScrollState != SCROLL_STATE_CLOSE) {
            mOnItemSlideListenerProxy.onSlideClose(this, getItemCustomView().getLeft() < 0 ? ListMenuItem.DIRECTION_LEFT : ListMenuItem.DIRECTION_RIGHT);
        }
        postInvalidate();
        mScrollState = SCROLL_STATE_CLOSE;
        enableBackgroundDrawable();
    ...
    private void disableBackgroundDrawable() {
        if (mNormalCustomBackgroundDrawable != null) {
            UIUtil.setBackgroundDrawable(getItemCustomView(), mNormalCustomBackgroundDrawable);
        }
        if (mNormalListSelectorDrawable != null) {
            UIUtil.setBackgroundDrawable(getItemLeftBackGroundLayout(), mNormalListSelectorDrawable);
            UIUtil.setBackgroundDrawable(getItemRightBackGroundLayout(), mNormalListSelectorDrawable);
        }
    }
    ...
    private void enableBackgroundDrawable() {
        if (mTotalCustomBackgroundDrawable != null) {
            UIUtil.setBackgroundDrawable(getItemCustomView(), mTotalCustomBackgroundDrawable);
        }

        if (mTotalListSelectorDrawable != null) {
            UIUtil.setBackgroundDrawable(getItemLeftBackGroundLayout(), mTotalListSelectorDrawable);
            UIUtil.setBackgroundDrawable(getItemRightBackGroundLayout(), mTotalListSelectorDrawable);
        }
    }
