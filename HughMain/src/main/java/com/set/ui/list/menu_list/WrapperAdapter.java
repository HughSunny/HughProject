...
class WrapperAdapter implements WrapperListAdapter, ItemMainLayout.OnItemSlideListenerProxy, View.OnClickListener,
    ...
    /* 用户自定义参数 */
    private Map<Integer, ListMenu> mMenuMap;
    /* SDLV */
    ...
    protected WrapperAdapter(Context context, SlideAndDragListView listView, ListAdapter adapter, Map<Integer, ListMenu> map) {
        mContext = context;
        mListView = listView;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            ListMenu menu = mMenuMap.get(type);
            if (menu == null) {
                throw new IllegalArgumentException("没有这个ViewType");
            }
            itemMainLayout.setParams(menu.getTotalBtnLength(ListMenuItem.DIRECTION_LEFT),
                    menu.getTotalBtnLength(ListMenuItem.DIRECTION_RIGHT), menu.isWannaOver());
     ...
     */
    private void createMenu(ListMenu menu, ItemMainLayout itemMainLayout) {
        if (menu.getTotalBtnLength(ListMenuItem.DIRECTION_LEFT) > 0) {
            for (int i = 0; i < menu.getMenuItems(ListMenuItem.DIRECTION_LEFT).size(); i++) {
                View v = itemMainLayout.getItemLeftBackGroundLayout().addMenuItem(menu.getMenuItems(ListMenuItem.DIRECTION_LEFT).get(i));
        ...
        if (menu.getTotalBtnLength(ListMenuItem.DIRECTION_RIGHT) > 0) {
            for (int i = 0; i < menu.getMenuItems(ListMenuItem.DIRECTION_RIGHT).size(); i++) {
                View v = itemMainLayout.getItemRightBackGroundLayout().addMenuItem(menu.getMenuItems(ListMenuItem.DIRECTION_RIGHT).get(i));
                v.setOnClickListener(this);
    ...
    public void onClick(View v) {
        if (mOnAdapterMenuClickListenerProxy != null) {
            int scroll = mOnAdapterMenuClickListenerProxy.onMenuItemClick(v, mSlideItemPosition,
                    ...
                    v.getTag(TAG_LEFT) != null ? ListMenuItem.DIRECTION_LEFT : ListMenuItem.DIRECTION_RIGHT);
            switch (scroll) {
                case ListMenu.ITEM_NOTHING:
                    break;
                case ListMenu.ITEM_SCROLL_BACK:
                    //归位
                    returnSlideItemPosition();
                    break;
                case ListMenu.ITEM_DELETE_FROM_BOTTOM_TO_TOP:
                    if (mSlideItemPosition != -1) {
                        ItemMainLayout itemMainLayout = (ItemMainLayout) mListView.getChildAt(mSlideItemPosition - mListView.getFirstVisiblePosition());
                        if (itemMainLayout != null) {
