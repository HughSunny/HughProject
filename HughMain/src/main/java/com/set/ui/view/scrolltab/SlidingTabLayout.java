...
public abstract class SlidingTabLayout extends HorizontalScrollView {
	...
	public abstract View getWidgetView(String key, int position);

	public abstract void onTabWidgetChange(int position);
    ...
    public void setNeedIndicator(boolean needIndicator) {
        mTabStrip.setNeedIndicator(needIndicator);
    }
    ...
    private void populateTabStrip() {
        ...
        if(mViewPager != null) {
            ...
            for (int i = 0; i < adapter.getCount(); i++) {
            	View tabView = addTabItem(adapter.getPageTitle(i).toString(),i);
            ...
            for (int i = 0; i < titles.length; i++) {
                View tabView = addTabItem(titles[i],i);
                tabView.setOnClickListener(tabClickListener);
                String desc = mContentDescriptions.get(i, null);
                if (desc != null) {
                    tabView.setContentDescription(desc);
    ...
    private View addTabItem(String itemTitle, int position) {
        View tabView = null;
        TextView tabTitleView = null;
        tabView = getWidgetView(itemTitle, position);
        if(tabView == null) {
			...
			tabTitleView.setText(itemTitle);
