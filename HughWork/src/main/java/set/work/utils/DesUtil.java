...
public class DesUtil {
    ...
    public static void main(String[] args) {
        ...
        try {
            // 调用DES加密方法
            String encryString = DesUtil.encryptDES(text, key);
            System.out.println("DES加密结果： " + encryString);
            // 调用DES解密方法
            String decryString = DesUtil.decryptDES(encryString, key);
    ...
    private static String decryptDES(String decryptString, String decryptKey)
        ...
        byte[] base64byte = Base64Util.decode(decryptString.toCharArray());
        // 实例化IvParameterSpec对象，使用指定的初始化向量
        IvParameterSpec spec = new IvParameterSpec(iv);
        // 实例化SecretKeySpec类,根据字节数组来构造SecretKeySpec
